...
def test_selection_matches_previous():
    expected = {
        16: ([0], [4.9744415283203125]),
        17: ([0, 1], [4.282526016235352] * 2),
        20: ([0, 4], [2.7412118911743164] * 2),
        31: ([8, 12, 15], [4.686454772949219, 4.686454772949219, 4.227546215057373]),
    ...
    for n in (16, 17, 20, 31, 48, 93):
        ...
        features = rng.normal(size=(len(indices[::8]), 7)).astype(np.float32)
        ...
        assert [list(w.frame_indices) for w in uniform_windows(indices)] == [
            indices[s : s + 16] for s in starts
        ]


def test_reference_cdf_and_roundtrip(tmp_path):
    cdfs = local_video_cdfs([[0.0, 2.0, 4.0], [2.0, 4.0, 6.0]])
    np.testing.assert_array_equal(cdfs[1], [0.0, 2.0])
    np.testing.assert_array_equal(cdfs[2], [2.0, 4.0])
    gs = StableGaussianParams(np.zeros(2), np.eye(2), np.array([0.0, 2.0]))
    gt = StableGaussianParams(np.zeros(2), np.eye(2), np.array([0.0, np.inf]))
    bundle = ReferenceBundle(gs, gt, gs, cdfs, dict(schema=SCHEMA))
    path = tmp_path / "reference.npz"
    save_bundle(path, bundle)
    result = load_bundle(path).score_video([1.0], [np.inf], [2.0])
    assert (
        result["global_score"] == 0.75
        and result["local_score"] == 1.0
        and result["final_score"] == 0.875
    )
    with pytest.raises(ValueError):
        percentile([np.nan], [0.0, 1.0])


def test_balanced_fit_preserves_covariance():
    rng = np.random.default_rng(29)
    videos = [rng.normal(size=(8, 4)) for _ in range(3)]
    params = fit_gaussian(videos)
    expected = np.cov(np.concatenate(videos).T) + 1e-5 * np.eye(4)
    np.testing.assert_allclose(
        params.whitening @ params.whitening.T, np.linalg.inv(expected), rtol=1e-12, atol=1e-12
    )
...
def test_window_mean_preserves_legacy_group_reduction():
    ...
    different = False
    for _ in range(100):
        values = rng.integers(0, 2001, 3) / 2000.0
        expected = (
            pd.DataFrame({"video": ["v"] * 3, "score": values})
            .groupby("video")
            .score.mean()
            .iloc[0]
        )
