...
def test_config_contract():
    ...
    with pytest.raises(ValueError):load_paper_config(path,['runtime.deivce=cpu'])
    with pytest.raises(ValueError):load_paper_config(path,['encoder.batch_size=64'])
    config=load_paper_config(path);config['unknown']={}
    with pytest.raises(ValueError):validate_paper_config(config)
    assert load_paper_config(path,['method.local_enabled=false'])['method']['local_enabled'] is False


def make_cli_case(tmp_path):
    spec=importlib.util.spec_from_file_location('paper_cli',ROOT/'scripts/run.py')
    cli=importlib.util.module_from_spec(spec);spec.loader.exec_module(cli)
    params=StableGaussianParams(np.zeros(2),np.eye(2),np.array([0.,2.]))
    bundle=ReferenceBundle(params,params,params,local_video_cdfs([[0.,2.,4.],[2.,4.,6.]]),dict(schema=SCHEMA,dataset='test'))
    reference=tmp_path/'test.npz';save_bundle(reference,bundle);digest=file_digest(reference)
    manifest=tmp_path/'manifest.json';manifest.write_text(json.dumps(dict(status='completed',references=[dict(dataset='test',sha256=digest)])))
    ...
    windows=pd.DataFrame([dict(video_id=v,dataset='test',subset=s,source_model=g,video_path=v+'.mp4',window_id=0,
        reference_sha256=digest,global_spatial_raw=x,global_temporal_raw=x,local_raw=x) for v,s,g,x in [('r','real','real',2.),('f','annotated','g',-1.)]])
    raw=tmp_path/'windows.csv';windows.to_csv(raw,index=False)
    run=tmp_path/'run'
    base=['run.py','--config',str(path),'replay','--window-scores',str(raw),'--output',str(run)]
...
def test_cli_replay_evaluate_and_dry_run(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(sys,'argv',base+['--dry-run']);cli.main()
    assert not run.exists()
    monkeypatch.setattr(sys,'argv',base);cli.main()
    ...
    monkeypatch.setattr(sys,'argv',['run.py','--config',str(path),'evaluate','--run-dir',str(run),'--pairs',str(pairs)])
    cli.main()
    result=pd.read_csv(run/'generator_metrics.csv')
    assert result.iloc[0].auc==1. and result.iloc[0].real_positive_ap==1.
    ...
    monkeypatch.setattr(sys,'argv',base+['--resume']);cli.main()
...
def test_interrupted_replay_resumes_without_recomputing_first_video(tmp_path,monkeypatch):
    ...
    def fail_second(frame,*args,**kwargs):
        vid=frame.video_id.iloc[0];calls.append(vid)
        if vid=='f':raise RuntimeError('模拟中断')
        return original(frame,*args,**kwargs)
    monkeypatch.setattr(workflow,'replay_scores',fail_second)
    monkeypatch.setattr(sys,'argv',base)
    with pytest.raises(RuntimeError,match='模拟中断'):cli.main()
    ...
    def observe(frame,*args,**kwargs):
        calls.append(frame.video_id.iloc[0]);return original(frame,*args,**kwargs)
    ...
    monkeypatch.setattr(sys,'argv',base[:3]+['--set','method.local_enabled=false']+base[3:]+['--resume'])
    with pytest.raises(ValueError,match='配置'):cli.main()
...
def test_components_cli_and_explicit_variant(tmp_path,monkeypatch):
    ...
    command=['run.py','--config',str(path),'components','--run-dir',str(run),'--pairs',str(pairs),'--output',str(output)]
    monkeypatch.setattr(sys,'argv',command+['--dry-run']);cli.main()
    assert not output.exists()
    monkeypatch.setattr(sys,'argv',command);cli.main()
    with pytest.raises(ValueError,match='variant'):read_paper_scores(output)
    ...
    with pytest.raises(ValueError,match='不存在'):read_paper_scores(output,'not_a_variant')
    snapshots=json.loads((output/'run_manifest.json').read_text())['input_manifest_snapshots']
    assert len(snapshots)==1
    for digest,record in snapshots.items():assert file_digest(output/record['path'])==digest
...
def test_evaluation_lock_and_interrupted_publication(tmp_path,monkeypatch):
    ...
    cli,path,windows,run,base=make_cli_case(tmp_path)
    monkeypatch.setattr(sys,'argv',base);cli.main()
    pairs=tmp_path/'pairs.csv';windows[['video_id','dataset','subset']].assign(generator='g').to_csv(pairs,index=False)
    command=['run.py','--config',str(path),'evaluate','--run-dir',str(run),'--pairs',str(pairs)]
    with (run/'.run.lock').open('a') as lock:
        fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
        monkeypatch.setattr(sys,'argv',command)
        with pytest.raises(BlockingIOError):cli.main()
    assert not (run/'stages/evaluate').exists()
    original=artifacts.publish_stage_file;calls=[]
    def interrupt(source,destination,digest):
        calls.append(destination.name)
        if len(calls)==2:raise RuntimeError('模拟评价发布中断')
        original(source,destination,digest)
    monkeypatch.setattr(artifacts,'publish_stage_file',interrupt)
    with pytest.raises(RuntimeError,match='发布中断'):cli.main()
    ...
    def forbid(*args,**kwargs):raise AssertionError('准备好的表不应重新计算')
    monkeypatch.setattr(evaluation.tables,'evaluate_fixed_pairs',forbid)
    monkeypatch.setattr(sys,'argv',command+['--resume']);cli.main()
    ...
    with pytest.raises(ValueError,match='身份改变'):cli.main()
...
def test_evaluation_keeps_unowned_existing_results(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(sys,'argv',base);cli.main()
    pairs=tmp_path/'pairs.csv';windows[['video_id','dataset','subset']].assign(generator='g').to_csv(pairs,index=False)
    existing=run/'generator_metrics.csv';existing.write_text('user-owned result\n')
    monkeypatch.setattr(sys,'argv',['run.py','--config',str(path),'evaluate','--run-dir',str(run),'--pairs',str(pairs)])
    with pytest.raises(FileExistsError,match='拒绝覆盖'):cli.main()
...
def test_compact_score_reference_requires_declared_schema(tmp_path,monkeypatch):
    ...
    with pytest.raises(ValueError,match='schema'):read_paper_scores(run)
    manifest['schema']='paper_score_reference_v1';paper_json(run/'run_manifest.json',manifest)
