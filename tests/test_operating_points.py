...
def test_collinear_roc_thresholds_are_not_discarded():
    ...
    frame = pd.DataFrame(
        dict(subset=["real"] * 2000 + ["annotated"] * 2000, final_score=np.r_[scores, scores])
    )
    metrics = binary_metrics(frame)
    assert metrics["fake_tpr_at_0_1pct_real_fpr"] == 0.001
    assert metrics["fake_tpr_at_1pct_real_fpr"] == 0.01
    assert metrics["real_fpr_at_95pct_fake_tpr"] == 0.95
