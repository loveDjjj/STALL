...
def fixture():
    ...
    for domain in ('comgenvid','videofeedback','genvideo'):
        for i in range(4):
            vid=f'{domain}:{i}';subset='real' if i<2 else 'annotated'
            rows.append(dict(video_id=vid,dataset=domain,subset=subset,source_model='real' if i<2 else 'g',final_score=float(4-i)))
            pairs.append(dict(video_id=vid,dataset=domain,subset=subset,generator='g'))
    scores=pd.DataFrame(rows)
    # 每个内容组同时包含一个real和一个fake，验证不是逐行独立重抽样。
    groups=pd.Series({r['video_id']:r['dataset']+':'+str(int(r['video_id'].split(':')[1])%2) for r in rows})
...
def test_missing_and_relabeled_scores_rejected():
    ...
    changed=scores.copy();changed.loc[0,'subset']='annotated'
...
def test_generator_average_weights_domains_by_cell_count():
    ...
    extra['video_id']=extra.video_id+':g2';extra['source_model']='g2'
    extra_pairs=pd.concat([pairs[(pairs.dataset=='genvideo')&(pairs.subset=='real')],
        extra[['video_id','dataset','subset']]],ignore_index=True).assign(generator='g2')
    scores=pd.concat([scores,extra],ignore_index=True);pairs=pd.concat([pairs,extra_pairs],ignore_index=True)
    groups=pd.concat([groups,pd.Series({r.video_id:r.video_id for r in extra.itertuples()})])
    baseline=scores.copy();mask=baseline.dataset=='genvideo';baseline.loc[mask,'final_score']=-baseline.loc[mask,'final_score']
    delta,_=paired_source_contrast(scores,baseline,pairs,groups,iterations=10,include_generator_average=True)
    assert delta.query("dataset=='Average' and metric=='auc'").delta.iloc[0]==pytest.approx(.5)
