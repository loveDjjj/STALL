...
def test_shared_model_scores_match_direct_whitening():
    ...
    assert torch.equal(values['local_d2'],l2_normalized_second_order(torch.from_numpy(p)).reshape(2,-1,5))
    models={}
    for name,x in values.items():
        d=x.shape[-1]
        models[name]={str(i):StableGaussianParams(rng.normal(size=d),rng.normal(size=(d,d)),np.empty(0)) for i in range(2)}
    ...
    for name,entries in models.items():
        ...
        for key,m in entries.items():
            expected=-.5*((((x-m.mean)@m.whitening)**2).sum(-1)+x.shape[-1]*np.log(2*np.pi)).mean(1)
