...
def test_weighted_metrics_match_sklearn_with_ties():
    ...
    for n in (8, 50, 250):
        y = np.arange(n) % 2 == 0
        s = rng.integers(0, 7, n).astype(float)
        m = RankedMetrics(y, s)
        for _ in range(30):
            w = rng.poisson(1, n)
...
def test_shared_source_contrasts_match_existing_bootstrap():
        ...
        for i in range(8):
            ...
            rows.append(
                dict(
                    ...
                    source_model="real" if real else gen,
            ...
            for i in ids:
                pairs.append(
                    dict(
                        video_id=f"{d}:{i}",
                        dataset=d,
                        subset="real" if i < 4 else "annotated",
                        generator=gen,
                    )
                )
    a = pd.DataFrame(rows)
    b = a.copy()
    b.final_score = rng.integers(0, 5, len(b))
    ...
    expected = (
        point.merge(ci, on=["dataset", "metric"])
        .replace({"dataset": {"Macro-3": "Average"}})
        .set_index(["dataset", "metric"])
    )
    actual = shared_contrasts(
        {"a": a, "b": b, "a_copy": a.copy(), "b_copy": b.copy()},
        pairs,
        groups,
        {"simple": {"a": 1, "b": -1}, "interaction": {"a": 1, "b": -1, "a_copy": -1, "b_copy": 1}},
        iterations=30,
    )
    ...
    found = (
        actual[actual.contrast.eq("simple")].set_index(["dataset", "metric"]).loc[expected.index]
    )
