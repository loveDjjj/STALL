...
def test_official_window_uses_second_rng_draw():
    ids=list(range(0,240,3));rng=np.random.RandomState(42)
...
def test_zero_temporal_excluded_and_right_ties():
    p=dict(mu_spat=np.zeros(2),W_spat=np.eye(2),mu_temp=np.zeros(2),W_temp=np.eye(2),
           calib_ll_spat=np.array([[-5.,-4.],[-3.,-2.]]),calib_ll_temp=np.array([[-5.,-4.],[-3.,-2.]]))
