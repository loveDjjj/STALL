...
def fit_case(tmp_path):
    ...
    for role in ("fit", "cdf", "evaluation", "threshold"):
        ...
        for i in range(2):
            vid = f"{role}_{i}"
            video = tmp_path / f"{vid}.mp4"
            video.write_bytes(vid.encode())
            row = dict(
                video_id=vid,
                video_path=str(video),
                source_group=vid,
                dataset="test" if role in ("fit", "evaluation") else "vatex",
                subset="real",
                split=role,
                downsample_idxs=json.dumps(list(range(32))),
            )
            if role == "fit":
                asset = tmp_path / f"{vid}.npz"
                rng = np.random.default_rng(i)
                g = rng.normal(size=(3, 16, 1024)).astype(np.float32)
                g[:, 1] = g[:, 0]  # Global严格零T1不进入拟合。
                raw = rng.normal(size=(256, 1024)).astype(np.float32)
                raw[0] = 0  # Local零向量参与。
                np.savez_compressed(
                    asset,
                    video_id=vid,
                    ...
                    frame_indices=np.asarray(
                        [w.frame_indices for w in uniform_windows(range(32), 3)]
                    ),
                )
                row.update(feature_asset=str(asset), feature_asset_sha256=file_digest(asset))
            records.append(row)
        paths[role] = tmp_path / f"{role}.csv"
        pd.DataFrame(records).to_csv(paths[role], index=False)
...
def test_roles_and_cached_vector_identity(fit_case, monkeypatch):
    ...
    assert (
        stats["gs"]["observations"] == 96
        and stats["gt"]["observations"] == 84
        and stats["lt"]["observations"] == 512
    )
    assert np.count_nonzero(observed[2][0][0]) == 0
    np.testing.assert_allclose(np.linalg.norm(observed[2][0][1:], axis=1), 1, atol=2e-7)
    broken = pd.read_csv(paths["cdf"])
    broken.loc[0, "source_group"] = "fit_0"
    broken.to_csv(paths["cdf"], index=False)
    with pytest.raises(ValueError, match="相同source_group"):
        prepare_fit_inputs(ROOT, config, "test", *paths.values())


def test_fit_rejects_fake_and_changed_asset(fit_case):
    config, _, paths = fit_case
    original = pd.read_csv(paths["fit"])
    bad = original.copy()
    bad.loc[0, "subset"] = "annotated"
    bad.to_csv(paths["fit"], index=False)
    with pytest.raises(ValueError, match="真实"):
        prepare_fit_inputs(ROOT, config, "test", *paths.values())
    original.loc[0, "feature_asset_sha256"] = "0" * 64
    original.to_csv(paths["fit"], index=False)
...
def test_fit_cli_pause_resume_and_export(fit_case, tmp_path, monkeypatch):
    ...
    for role, path in paths.items():
        args += [f"--{role}-manifest", str(path)]
    fits = []
    scored = []
    fail = [True]

    def fit_model(videos, **kwargs):
        fits.append(len(videos))
        return StableGaussianParams(np.zeros(1024), np.eye(1024), np.empty(0))
    ...
    class Scorer:
        ...
        def score_global_first_window(self, path, indices):
            return dict(windows=[dict(global_spatial_raw=0.0, global_temporal_raw=float("inf"))])

        def score_raw(self, path, indices, **kwargs):
            scored.append(Path(path).stem)
            if Path(path).stem == "cdf_1" and fail[0]:
                raise RuntimeError("模拟CDF中断")
    ...
    monkeypatch.setattr(sys, "argv", args + ["--stop-after", "gaussian"])
    cli.main()
    assert json.loads((run / "status.json").read_text())["status"] == "paused"
    assert fits == [2, 2, 2] and not (run / "references").exists()
    with pytest.raises(ValueError, match="未完成CDF"):
        verified_fit_bundle(run)
    monkeypatch.setattr(sys, "argv", args + ["--resume"])
    with pytest.raises(RuntimeError, match="模拟CDF中断"):
        cli.main()
    ...
    np.testing.assert_array_equal(bundle.local_cdfs[3], [1.0, 1.0])
    before = file_digest(run / "references/test.npz")
    cli.main()
