...
def example_worker(job,report):
    ...
    if job.get('fail'):raise RuntimeError('测试工作进程失败')
...
def test_shard_identity_and_config():
    ...
    for override in ('runtime.devices=[cuda:0,cuda:0]','runtime.devices=[cpu,cuda:0]','runtime.devices=[cuda:3]'):
        with pytest.raises(ValueError):load_paper_config(ROOT/'configs/paper.yaml',[override])
    with pytest.raises(ValueError):row_numbers(frame,[1,2])
    with pytest.raises(ValueError):row_numbers(frame,[1,2,3,4,4])


def test_workers_wait_for_exit_and_reap_failure(tmp_path):
    before={p.pid for p in multiprocessing.active_children()}
    jobs=[dict(name=str(i),total=1,output=str(tmp_path),delay=.1) for i in range(2)]
    progress=[]
    run_workers(example_worker,jobs,tmp_path,'success',lambda stage,n,total,vid:progress.append(n))
    assert progress[-1]==2
    assert all(json.loads(p.read_text())['status']=='completed' for p in (tmp_path/'workers/success').glob('*/status.json'))
    jobs[0]['fail']=True;jobs[1]['delay']=10.
    started=time.monotonic()
    with pytest.raises(RuntimeError,match='失败'):run_workers(example_worker,jobs,tmp_path,'failure')
...
def test_missing_worker_checkpoint_is_not_silently_rescored(tmp_path):
    ...
    with pytest.raises(ValueError,match='缺失评分检查点'):
        score_manifest_checkpointed(tmp_path,{},frame,None,'ref',tmp_path,'id',{},require_cached=True)
