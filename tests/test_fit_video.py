...
def test_sampling_preserves_legacy_path():
    ...
    with pytest.raises(ValueError):sample_d2_positions(values[:200],identity)
...
def test_video_assets_resume_and_keep_legacy_identity(tmp_path,monkeypatch,row_indices,numbers):
    ...
    config['runtime']['device']='cpu';config['runtime']['decode_workers']=2
    rows=[]
    for i in range(2):
        path=tmp_path/f'{i}.mp4';path.write_bytes(b'video')
        rows.append(dict(video_id=f'v{i}',video_path=str(path),legacy_path=f'original/{i}.mp4',
            downsample_idxs=json.dumps(list(range(32)))))
    frame=pd.DataFrame(rows);calls=[];fail=[True]
    def arrays():
        t=np.arange(32,dtype=np.float32)
        # 让不同时间位置D2不同，能够检测采样位置或legacy_path变化。
        return {'global':np.broadcast_to(t[:,None],(32,1024)).copy(),
                'patch':np.broadcast_to(t[:,None,None,None]**3,(32,4,4,1024)).copy()}
    class Model:
        def __init__(self,*args,**kwargs):calls.append('model')
        def prepare_frames(self,frames):return frames
        def frames_to_global_patch_embeddings(self,frames,batch_size):
            assert batch_size==8
            calls.append('forward')
            if calls.count('forward')==2 and fail[0]:raise RuntimeError('模拟前向中断')
            return [arrays()]
    ...
    with pytest.raises(RuntimeError,match='中断'):
        prepare_fit_assets(tmp_path,config,frame,states,tmp_path,'run',row_indices=row_indices)
    assert (tmp_path/f'fit_features/{numbers[0]:06d}.json').exists() and not (tmp_path/f'fit_features/{numbers[1]:06d}.json').exists()
    fail[0]=False
    result=prepare_fit_assets(tmp_path,config,frame,states,tmp_path,'run',row_indices=row_indices)
    before=len(calls)
    again=prepare_fit_assets(tmp_path,config,frame,states,tmp_path,'run',row_indices=row_indices)
    ...
    for row in result.itertuples():
        with np.load(row.feature_asset) as z:
            ...
            source=arrays()['patch'];patch=torch.from_numpy(np.stack([source[idx] for idx in z['frame_indices']]))
            raw=(patch[:,2:]-2.*patch[:,1:-1]+patch[:,:-2]).reshape(-1,1024)
            np.testing.assert_array_equal(z['raw_d2'],sample_d2_positions(raw,row.legacy_path).numpy())
    (tmp_path/f'fit_features/{numbers[0]:06d}.npz').write_bytes(b'corrupt')
    with pytest.raises(ValueError,match='损坏'):prepare_fit_assets(tmp_path,config,frame,states,tmp_path,'run',row_indices=row_indices)
...
def test_legacy_configs_keep_read_compatibility():
    ...
    config=load_paper_config(ROOT/'configs/paper.yaml')
    config.pop('fit')
    for field in ('decode_workers','prefetch_depth','prefetch_memory_mb','devices','prefetch_enabled'):config['runtime'].pop(field)
