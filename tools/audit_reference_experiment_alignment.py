...
CHECKS = [
    ...
    },
    {
        ...
        "alpha_stalled_counterpart": "ComGenVid lag-1、multi-lag、motion-hard/soft、同网格二阶时序消融",
        "required_paths": [
        ...
        "status_if_exists": "partial_but_sufficient",
        "gap": "未完全复刻 D=1/2/3/4 有限差分；但已证明我们的同网格二阶 patch 证据优于多种局部时序替代。",
        "next_action": "不优先补；如需更像原文，可只在 ComGenVid 增加 D=3/4 patch temporal 对照。",
        "priority": "P2",
    },
    {
    ...
    },
    {
        ...
        "alpha_stalled_counterpart": "当前已有 release 复现/结果说明，但未系统审计 D3 protocol",
        "required_paths": [
            "results/stall_repro_comparison.md",
        ],
        "status_if_exists": "partial_dirty_file_needs_review",
        "gap": "该文件当前有未提交修改，且 D3 protocol audit 不是 Alpha-STALLED 主创新所必需。",
        "next_action": "提交前单独审查 `results/stall_repro_comparison.md` diff；不要混入当前补充实验提交。",
        "priority": "P2",
    },
    {
        "reference_section": "E",
        "reference_item": "inference time / memory analysis",
        "alpha_stalled_counterpart": "runtime/storage audit + CSV-stage runtime benchmark",
        "required_paths": [
            "results/journal_experiments/runtime_storage_audit/runtime_storage_audit.md",
            "results/journal_experiments/runtime_benchmark/csv_stage_runtime_benchmark.md",
        ],
        "status_if_exists": "partial_csv_runtime_complete_end_to_end_pending",
        "gap": "已有 storage footprint、历史日志片段和可复现 CSV-stage 计时；仍缺原始视频解码、DINOv3 embedding、patch prefill、全量 patch eval 的固定环境端到端 benchmark。",
        "next_action": "若论文需要完整 runtime 表，固定 GPU/batch/cache 状态重跑视频级阶段；当前 CSV-stage benchmark 可直接报告为轻量后处理成本。",
        "priority": "P1",
    },
    {
        "reference_section": "D.7 / Fig. 19-20",
        "reference_item": "qualitative examples",
        "alpha_stalled_counterpart": "patch anomaly case visualization + failure case audit",
        "required_paths": [
        ...
        "status_if_exists": "complete_without_raw_keyframes",
        "gap": "已有 patch anomaly map；若要更直观，需要再叠加原视频关键帧。",
        "next_action": "版面允许时补关键帧；当前统计和 patch map 已足够支撑 failure-mode 选择。",
        "priority": "P2",
    },
    {
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "results/journal_experiments/reference_alignment_audit",
    )
