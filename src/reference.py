...
def percentile(values, reference, *, allow_positive_infinity=False):
    ...
    for x in (values, reference):
        if (
            np.isnan(x).any()
            or np.isneginf(x).any()
            or (not allow_positive_infinity and np.isposinf(x).any())
        ):
...
def window_mean(values):
    ...
    if values.ndim != 1 or not len(values) or not np.isfinite(values).all():
        raise ValueError("窗口均值需要非空有限向量")
    return float(
        pd.Series(values).groupby(np.zeros(len(values), dtype=np.int8), sort=False).mean().iloc[0]
    )
...
def fit_gaussian(videos, *, ridge=1e-5, diagonal=False):
    ...
    weights = np.concatenate([np.full(len(x), 1.0 / len(x)) for x in videos])
    ...
    whitening = eigenvectors / np.sqrt(np.maximum(eigenvalues, 0.0) + ridge)
...
class ReferenceBundle:
    ...
    def validate(self):
        ...
        for params in (self.global_spatial, self.global_temporal, self.local_temporal):
            if (
                params.mean.ndim != 1
                or params.whitening.ndim != 2
                or params.whitening.shape[0] != len(params.mean)
            ):
    ...
    def score_video(
        ...
        *,
        ...
        gs, gt, local = (
            ...
            for x in (global_spatial_raw, global_temporal_raw, local_raw)
        )
        ...
        final = (
            global_weight * g + (1 - global_weight) * l
        ...
        return dict(
            global_score=g,
            global_spatial_score=window_mean(s),
            global_temporal_score=window_mean(t),
            local_raw=q,
            local_score=l,
            final_score=final,
            effective_k=len(local),
        )
...
def save_bundle(path, bundle):
    bundle.validate()
    arrays = {
        "metadata": np.asarray(json.dumps(bundle.metadata, sort_keys=True, ensure_ascii=False))
    }
    for name, params in [
        ("gs", bundle.global_spatial),
        ("gt", bundle.global_temporal),
        ("lt", bundle.local_temporal),
    ]:
        arrays[name + "_mean"] = params.mean
        arrays[name + "_whitening"] = params.whitening
        if name != "lt":
            arrays[name + "_cdf"] = params.calibration_raw
    ...
    with np.load(path, allow_pickle=False) as z:
        parameters = [
            StableGaussianParams(
                z[n + "_mean"].copy(),
                z[n + "_whitening"].copy(),
                z[n + "_cdf"].copy() if n != "lt" else np.empty(0),
            )
            for n in ("gs", "gt", "lt")
        ]
        result = ReferenceBundle(
            *parameters,
            {k: z[f"local_cdf_k{k}"].copy() for k in (1, 2, 3)},
