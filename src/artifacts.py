...
def _normalize_json(value: Any, *, nonfinite_error: str) -> Any:
    """保持产物JSON表示：键转字符串、元组转列表，仅允许正无穷字符串。"""
    if isinstance(value, dict):
        return {
            str(key): _normalize_json(item, nonfinite_error=nonfinite_error)
            for key, item in value.items()
        }
    if isinstance(value, (list, tuple)):
        return [_normalize_json(item, nonfinite_error=nonfinite_error) for item in value]
    if isinstance(value, float) and not math.isfinite(value):
        if value == math.inf:
            return "Infinity"
        raise ValueError(nonfinite_error)
    return value


def paper_json(path: str | Path, value: dict[Any, Any]) -> None:
    ...
    with tempfile.NamedTemporaryFile(
        ...
        try:
            json.dump(
                _normalize_json(value, nonfinite_error="产物中出现NaN或负无穷"),
                stream,
                indent=2,
                ensure_ascii=False,
                allow_nan=False,
...
def checkpoint_write(path: str | Path, identity: str, payload: dict[Any, Any]) -> None:
    """先按真实JSON表示归一化，再保存载荷hash，避免Infinity表示改变校验值。"""
    payload = _normalize_json(payload, nonfinite_error="checkpoint包含非法非有限值")
    paper_json(
        Path(path),
        dict(
            schema="paper_checkpoint_v1",
            identity=identity,
            payload=payload,
            payload_sha256=config_digest(payload),
        ),
    )
...
def publish_stage_file(source, destination, expected_hash):
    ...
    if file_digest(source) != expected_hash:
        raise ValueError("阶段暂存产物hash改变")
    if destination.is_symlink():
        raise ValueError("拒绝发布到既有符号链接")
    if destination.exists():
        if file_digest(destination) != expected_hash:
            raise ValueError("已发布产物与阶段内容不一致，拒绝覆盖")
        return
...
def resume_paper_run(root, config, inputs, directory, stage):
    ...
    if manifest.get("action") != stage:
        raise ValueError("恢复阶段不匹配或旧run没有恢复合同")
    for name, digest in manifest["code_sha256"].items():
        if (
            file_digest(root / name) != digest
            or file_digest(directory / "code_snapshot" / name) != digest
        ):
            raise ValueError("恢复源码改变，请使用原源码快照或新建run")
    complete = (
        stage in manifest.get("completed_steps", [])
        and (directory / "artifact_manifest.json").is_file()
    )
    if complete:
        state = json.loads((directory / "artifact_manifest.json").read_text())
        if state.get("status") != "completed":
            raise ValueError("完成标记与产物状态不一致")
        for name, digest in state["artifacts"].items():
            if file_digest(directory / name) != digest:
                raise ValueError("已完成run的产物改变")
...
def create_paper_run(root, config, command, inputs, output=None):
    ...
    directory = (
        Path(output)
        if output
        else root / config["output"]["root"] / f"{config['protocol']['id']}__{stamp}__{digest[:8]}"
    )
    ...
    for name, source_digest in code_hashes.items():
        ...
        if file_digest(target) != source_digest or file_digest(root / name) != source_digest:
            raise ValueError("保存run源码期间文件改变")
    ...
    def freeze_inputs(value):
        ...
        if value.get("manifest_sha256") and value.get("path"):
            source_path = Path(value["path"]) / "run_manifest.json"
            expected = value["manifest_sha256"]
        elif value.get("source_manifest_sha256") and value.get("source_run"):
            source_path = Path(value["source_run"]) / "run_manifest.json"
            expected = value["source_manifest_sha256"]
        if source_path is not None:
            if file_digest(source_path) != expected:
                raise ValueError("输入run的manifest在消费期间改变")
            target = directory / "input_manifests" / f"{expected}.json"
            target.parent.mkdir(exist_ok=True)
            if not target.exists():
                shutil.copy2(source_path, target)
            if file_digest(target) != expected:
                raise ValueError("输入manifest副本hash不符")
            input_snapshots[expected] = dict(
                source=str(source_path), path=str(target.relative_to(directory))
            )
    ...
    manifest = dict(
        ...
        git_dirty=bool(git_state.strip()),
        git_diff_sha256=file_digest(directory / "code_snapshot/git_tracked.patch"),
        environment=versions,
        created_utc=stamp,
        input_manifest_snapshots=input_snapshots,
        completed_steps=[],
        scope="paper-v1; stages are recorded separately",
    )
...
class PaperLog:
    ...
    def __init__(self, directory):
        self.path = Path(directory) / "logs/console.log"
    ...
    def __enter__(self):
        ...
        sys.stdout = Tee(self.stdout, self.log)
        sys.stderr = Tee(self.stderr, self.log)
        return self
...
def finish_paper_stage(directory, stage):
    ...
    manifest = json.loads((directory / "run_manifest.json").read_text())
    if stage not in manifest["completed_steps"]:
        manifest["completed_steps"].append(stage)
    paper_json(directory / "run_manifest.json", manifest)
    artifacts = {
        str(p.relative_to(directory)): file_digest(p)
        for p in directory.rglob("*")
        if p.is_file()
        and p.name not in ("artifact_manifest.json", "status.json", ".run.lock")
        and p.suffix not in (".log", ".tmp")
    }
    paper_json(
        directory / "artifact_manifest.json",
        dict(status="completed", stage=stage, artifacts=artifacts),
    )
    paper_json(
        directory / "status.json",
        dict(status="completed", stage=stage, completed_steps=manifest["completed_steps"]),
    )
...
def read_paper_scores(directory, variant=None):
    ...
    if not steps & {"score", "replay", "components", "import_reference"}:
        raise ValueError("该run没有完成评分")
    if "import_reference" in steps and manifest.get("schema") != "paper_score_reference_v1":
        raise ValueError("分数参考schema错误")
    if components and variant is None:
        raise ValueError("组件run必须指定variant")
    if not components and variant is not None:
        raise ValueError("普通评分run不接受variant")
    if manifest.get("code_snapshot"):
        for name, digest in manifest["code_sha256"].items():
            if file_digest(directory / manifest["code_snapshot"] / name) != digest:
                raise ValueError("run源码快照改变")
    for digest, snapshot in manifest.get("input_manifest_snapshots", {}).items():
        if file_digest(directory / snapshot["path"]) != digest:
            raise ValueError("输入manifest快照改变")
    state = json.loads((directory / "artifact_manifest.json").read_text())
    name = "video_scores.csv.gz" if components else "video_scores.csv"
    path = directory / name
    if state.get("status") != "completed" or state["artifacts"].get(name) != file_digest(path):
        raise ValueError("评分产物不完整或改变")
    frame = pd.read_csv(path, float_precision="round_trip", dtype={"video_id": str})
    if components:
        if variant not in set(frame.variant):
            raise ValueError("组件variant不存在")
        frame = frame[frame.variant == variant].copy()
    return frame, dict(
        path=str(directory),
        variant=variant,
        manifest_sha256=file_digest(directory / "run_manifest.json"),
        scores_sha256=file_digest(path),
    )
