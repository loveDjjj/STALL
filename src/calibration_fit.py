"""为 Global+Local 方法创建 patch 级校准参数（.npz）。
...
from branches.local_branch import patch_temporal_delta
from math_utils import apply_whitening, bottomk_mean, log_likelihood
...
def iter_real_patch_cache(
    ...
    for _, row in reals.iterrows():
        ...
        if cache_context.strict:
            ...
            if index_column not in row or is_missing_window(row[index_column]):
...
def build_patch_params(
    ...
    aggregation_region_size: int = 3,
    seed: int = 42,
    patch_temp_mode: str = "same_grid_second_order",
    match_radius: int = 2,
    top_m: int = 4,
...
def main():
    ...
    parser.add_argument("--csv", required=True, help="build_manifest.py 生成的 manifest")
    ...
    parser.add_argument("--aggregation", choices=["mean"], default="mean")
    ...
    parser.add_argument(
        "--patch-temp-mode",
        choices=["same_grid_lag1", "same_grid_second_order"],
        default="same_grid_second_order",
    )
