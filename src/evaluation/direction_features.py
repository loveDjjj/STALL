...
def direction_features(patches, *, epsilon=1e-8, gamma=8.):
    ...
    grid=p.reshape(w,t,side,side,d);motion=grid[:,1:]-grid[:,:-1]
    ...
    if not torch.isfinite(values).all():raise ValueError('SPLIT产生非有限分数')
...
def pooled_score(vectors,params):
    ...
    if x.ndim!=3 or not np.isfinite(x).all():raise ValueError('pooled方向格式无效')
    z=(x-params.mean)@params.whitening
    return -.5*(np.square(z).sum(-1)+params.whitening.shape[1]*np.log(2*np.pi)).mean(1)
...
def fit_source_balanced(videos,groups,ridge=1e-5):
    ...
    if len(videos)!=len(groups) or len(set(groups))<2:raise ValueError('源组支持不足')
    counts=Counter(groups);x=np.concatenate(videos).astype(np.float64)
    w=np.concatenate([np.full(len(v),1/(len(v)*counts[g])) for v,g in zip(videos,groups)])
    mu=np.average(x,axis=0,weights=w);cov=np.atleast_2d(np.cov(x.T,aweights=w,ddof=1))
