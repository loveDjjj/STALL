...
def official_short(root, domain, frame):
    ...
    if done["world_size"] != 1 or spec["window_frames"] != 8:
        raise ValueError("官方短视频分片/帧数不符")
    rows = []
    for i, r in enumerate(frame.to_dict("records")):
        score = checkpoint_read(directory / "raw" / f"{i:06d}.json", identity, r["video_id"])
        if score["frame_indices"] != json.loads(r["downsample_idxs"]):
            raise ValueError("官方1秒帧身份不符")
        rows.append(
            {k: r[k] for k in ("video_id", "dataset", "subset", "source_model", "video_path")}
            | dict(
                variant="official_single_window", final_score=score["final_score"], effective_k=1
            )
        )
...
def prepare_short_tables(root, domain):
    root = Path(root)
    output = root / "results/runs" / f"complete23_tables_{domain}_short8"
    if output.exists():
        return checked_scores(output)
    ...
    if ei["models"] != ci["models"] or ei["config"] != ci["config"]:
        raise ValueError("8帧评价和CDF参数不同")
    if ei["config"]["evidence_window_frames"] != 8:
        raise ValueError("短视频参考不是8帧")
    ...
    for variant, part in scores.groupby("variant"):
        tables.append(
            evaluate_fixed_pairs(part, pairs)["generator_metrics"].assign(variant=variant)
        )
    ...
    paper_json(
        ...
        dict(
            ...
            evaluation_manifest_sha256=file_digest(manifest),
...
def combine(root, output, scope, native_official=None):
    ...
    if scope in ("paper_filter", "paper_aligned"):
        audit = (
            "complete23_official_pairing_audit"
            if scope == "paper_aligned"
            else "videofeedback_dynamic_filter_audit"
        )
        filtered = pd.read_csv(
            root / "results/runs" / audit / "filtered_pairs.csv", keep_default_na=False
        )
        long_pairs = pd.concat(
            [long_pairs[long_pairs.dataset != "videofeedback"], filtered], ignore_index=True
        )
    pairs = pd.concat(
        [
            long_pairs,
            *[
                pd.read_csv(
                    root / "data/manifests/short_video" / d / "pairs.csv", keep_default_na=False
                )
                for d in short
            ],
        ],
    ...
    if len(cells) != 23:
        raise ValueError("完整协议不是23单元")
    manifests = []
    for d in ("comgenvid", "genvideo", "videofeedback"):
        manifests.append(
            pd.read_csv(
                root / "data/manifests/active" / d / "evaluation.csv", keep_default_na=False
            )
        )
        if d in short:
            manifests.append(
                pd.read_csv(
                    root / "data/manifests/short_video" / d / "evaluation.csv",
    ...
    if frame.video_id.duplicated().any():
        raise ValueError("跨时长评价身份重复")
    modes = {}
    source_hashes = {}
    for mode in ("official", "fc3", "uniform1", "fc1", "uniform3"):
        source = (
            Path(native_official)
            if mode == "official" and native_official
            else root / "results/runs" / f"paper_tables_all_{mode}"
        )
        long = checked_scores(source)
        source_hashes[str(source)] = file_digest(source / "manifest.json")
        variants = set(long.variant)
        parts = (
            [long]
            if mode == "official" and native_official
            else [long, *[s[s.variant.isin(variants)] for s in short.values()]]
        )
        combined = pd.concat(parts, ignore_index=True)
        combined = combined[combined.video_id.isin(set(pairs.video_id))]
        for variant, part in combined.groupby("variant"):
            if part.video_id.duplicated().any() or set(part.video_id) != set(pairs.video_id):
                raise ValueError(f"{mode}/{variant}身份不完整")
        modes[mode] = combined
    output.mkdir(parents=True, exist_ok=False)
    pairs.to_csv(output / "pairs.csv", index=False)
    frame.to_csv(output / "evaluation.csv", index=False)
    for mode, scores in modes.items():
        scores.to_csv(output / f"scores_{mode}.csv.gz", index=False)
    for study, definitions in STUDIES.items():
        cells_out = []
        summary = []
        for mode, variant, label in definitions:
            part = modes[mode]
            part = part[part.variant == variant]
            tables = evaluate_fixed_pairs(part, pairs)
            g = tables["generator_metrics"]
            if set(map(tuple, g[["dataset", "generator"]].to_numpy())) != cells:
                raise ValueError("消融未覆盖23单元")
            cells_out.append(g.assign(variant=variant, method=label, observation=mode))
            dataset = tables["dataset_metrics"]
            macro = tables["macro_metrics"].rename(columns={"scope": "dataset"})
            metrics = [c for c in dataset if c != "dataset"]
            average = pd.DataFrame([dict(dataset="Average", **g[metrics].mean().to_dict())])
            summary.append(
                pd.concat([dataset, average, macro]).assign(
                    variant=variant, method=label, observation=mode
                )
            )
    ...
    paper_json(
        ...
        dict(
            ...
            scope=scope,
            cells=23,
            official_native_source=str(Path(native_official).resolve())
            if native_official
            else None,
            unique_evaluation=len(frame),
            pair_rows=len(pairs),
            source_manifests=source_hashes,
            short_sources={
                d: file_digest(
                    root / "results/runs" / f"complete23_tables_{d}_short8/manifest.json"
                )
                for d in short
            },
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scope", choices=["coverage_only", "paper_filter", "paper_aligned"], required=True
    )
    parser.add_argument("--output", required=True)
    parser.add_argument("--native-official")
    args = parser.parse_args()
    combine(Path.cwd(), args.output, args.scope, args.native_official)
