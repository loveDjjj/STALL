...
def worker(job):
    ...
    baseline = pd.read_csv(directory / f"scores_{mode}.csv.gz", float_precision="round_trip")
    baseline = baseline[baseline.variant == variant]
    ...
    inputs = {
        p: file_digest(directory / p)
        for p in ["scores_fc3.csv.gz", f"scores_{mode}.csv.gz", "evaluation.csv", "pairs.csv"]
    }
    if (output / "manifest.json").exists():
        m = json.loads((output / "manifest.json").read_text())
        if m["inputs"] != inputs:
            raise ValueError("区间来源改变")
        return name
    point, interval = paired_source_contrast(
        candidate, baseline, pairs, groups, iterations=1000, seed=17, include_generator_average=True
    )
    output.mkdir(parents=True, exist_ok=True)
    point.to_csv(output / "deltas.csv", index=False)
    interval.to_csv(output / "intervals.csv", index=False)
    paper_json(
...
def main(directory, workers=4):
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        for name in pool.map(worker, [(str(directory), n) for n in CONTRASTS]):
            print("interval completed", name, flush=True)
    parts = []
    for name in CONTRASTS:
        p = directory / "intervals" / name
        d = pd.read_csv(p / "deltas.csv")
        c = pd.read_csv(p / "intervals.csv")
        parts.append(
            d.merge(c, on=["dataset", "metric"], validate="one_to_one").assign(
