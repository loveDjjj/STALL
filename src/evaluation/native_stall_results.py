...
def aggregate(root):
    ...
    for length, label, world_size in [(8, "8", 2), (16, "16", 2), (16, "supplement", 1)]:
        ...
        for rank in range(world_size):
            ...
            if (
                ...
                or done["world_size"] != world_size
                or spec["manifest_sha256"] != config_digest(frame.to_dict("records"))
            ):
                raise ValueError("原生官方检查点身份不符")
            if spec["frame_batch"] != 32 or spec["pad_tail"] is not False:
                raise ValueError("不是默认无补齐前向")
            identities[rank] = identity
            inputs[str(directory / f"identity_rank_{rank}.json")] = file_digest(
                directory / f"identity_rank_{rank}.json"
            )
        for i, r in enumerate(frame.to_dict("records")):
            payload = checkpoint_read(
                directory / "raw" / f"{i:06d}.json", identities[i % world_size], r["video_id"]
            )
            expected = json.loads(r["downsample_idxs"] if length == 8 else r["2_sec_idxs"])
            if payload["frame_indices"] != expected:
                raise ValueError("原生单窗帧身份不符")
            rows.append(
    ...
    old = (
        pd.concat(
            [
                pd.read_csv(
                    root / "results/runs/complete23_coverage_only/scores_official.csv.gz",
                    float_precision="round_trip",
                ),
                pd.read_csv(
                    root / "results/runs/paper_tables_all_official/video_scores.csv.gz",
                    float_precision="round_trip",
                ),
            ]
        )
    ...
    if indexed.index.duplicated().any() or not set(indexed.index).issubset(old.index):
        raise ValueError("默认前向缺少逐身份batch8对照")
    indexed["batch8_score"] = old.loc[indexed.index, "final_score"]
    indexed["difference"] = indexed.final_score - indexed.batch8_score
    output = root / "results/runs/complete23_native_stall_tables"
    ...
    comparison = indexed.groupby(["dataset", "window_frames"]).difference.agg(
        n="size",
        changed=lambda x: int(np.count_nonzero(x)),
        max_abs=lambda x: float(abs(x).max()),
        mean_abs=lambda x: float(abs(x).mean()),
    )
    comparison.to_csv(output / "batch_difference_summary.csv")
    for scope, path in [
        ("coverage_only", "results/runs/complete23_coverage_only"),
        ("paper_filter", "results/paper_complete"),
    ]:
        pairs = pd.read_csv(root / path / "pairs.csv")
        selected = scores[scores.video_id.isin(set(pairs.video_id))]
        tables = evaluate_fixed_pairs(selected, pairs)
        for key, table in tables.items():
            table.to_csv(output / f"{scope}_{key}.csv", index=False)
    paper_json(
