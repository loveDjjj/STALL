...
class BudgetEvidenceScorer(RawVideoScorer):
    ...
    def global_from_prepared(self, window, frames):
        ...
        return dict(
            ...
            global_models={
                "target": dict(gs=raw["global_spatial_raw"], gt=raw["global_temporal_raw"])
            },
        )

    def score_dense(self, plan, frames):
        start = time.perf_counter()
        raw = super().score_dense(plan, frames)
        return dict(
            ...
            windows=[
                dict(
                    ...
                    change=w["change"],
                    global_models={
                        "target": dict(gs=w["global_spatial_raw"], gt=w["global_temporal_raw"])
                    },
                    representations={"local_d2": {"target": w["local_raw"]}},
                )
