...
def prepare_reference(root, role):
    root = Path(root)
    source = root / "data/manifests/active/vatex" / f"{role}.csv"
    ...
    if output.exists():
        ...
        paper_json(
            ...
            dict(
                ...
                rule="same identity, 8fps 1sec seed42 first draw",
                rows=len(frame),
            ),
        )
    return output
...
def main():
    ...
    parser.add_argument(
        "--role", required=True, choices=["cdf", "threshold", "evaluation", "official"]
    )
    ...
    manifest = (
        prepare_reference(root, args.role)
        if args.role in ("cdf", "threshold")
        else root / "data/manifests/short_video" / args.domain / "evaluation.csv"
    )
