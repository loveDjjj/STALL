...
def fit_source_bank(root, config, output):
    ...
    frame["dataset"] = "vatex"
    frame["split"] = "fit"
    frame["source_group"] = "vatex:" + frame.source_id
    if len(frame) != 200 or frame.source_id.nunique() != 200:
        raise ValueError("源域拟合预算不是200独立ID")
    for role in ("cdf", "threshold"):
        other = pd.read_csv(root / "data/manifests/active/vatex" / f"{role}.csv")
        if set(frame.video_path) & set(other.video_path):
            raise ValueError("源域fit与CDF/threshold交叉")
    ...
    def progress(stage, n, total, vid):
        paper_json(
            output / "progress.json", dict(stage=stage, completed=n, total=total, video_id=vid)
        )
        if n % 16 == 0 or n == total:
            print(f"[source:{stage}] {n}/{total}", flush=True)
    ...
    for name, model in models.items():
        arrays[name + "_mean"] = model.mean
        arrays[name + "_whitening"] = model.whitening
    publish_arrays(output / "gaussians.npz", arrays)
    paper_json(
        output / "manifest.json",
        dict(
            status="gaussian_complete",
            identity=identity,
            statistics=statistics,
            models_sha256=file_digest(output / "gaussians.npz"),
            cdf_status="not_started",
        ),
    )
    return output


def run_shared_evidence(
    root,
    config,
    dataset,
    manifest,
    ...
    *,
    ...
    if baseline_only is None:
        baseline_only = (config["selection"]["name"], config["selection"]["k"]) in {
            ("feature_change", 1),
            ("uniform", 3),
        }
    paths = [
        root / "results/runs" / f"paper_fit_{dataset}" / "gaussians.npz",
    ...
    global_models = dict(
        target=(target["gs"], target["gt"]),
        source=(source["gs"], source["gt"]),
        official=(official["global_spatial"], official["global_t1"]),
    )
    local = {name: {"target": representations[name]} for name in REPRESENTATIONS}
    local["local_d2"] = mean_whitening_controls(
        source["lt"], target["lt"], representations["local_diagonal_d2"]
    )
    frame = pd.read_csv(manifest, keep_default_na=False)
    if frame.video_id.duplicated().any():
        raise ValueError("评价/CDF身份重复")
    roles = set(frame["split"])
    if roles == {"evaluation"}:
        if not frame.dataset.eq(dataset).all():
            raise ValueError("评价域与目标统计模型不一致")
    elif roles in ({"cdf"}, {"threshold"}):
        if not frame.subset.eq("real").all() or not frame.dataset.eq("vatex").all():
            raise ValueError("外部参考必须为VATEX真实视频")
        if roles == {"cdf"} and not include_uniform:
            raise ValueError("CDF任务必须包含Global Uniform首窗")
    else:
        raise ValueError("共享评分清单角色不明确")
    if limit is not None:
        frame = frame.iloc[:limit].copy()
    if file_digest(root / config["encoder"]["weights"]) != config["encoder"]["weights_sha256"]:
        raise ValueError("编码器权重错误")
    ...
    if baseline_only:
        ...
        config = dict(
            config,
            evidence_scope="baseline_budget",
            budget_scorer_sha256=file_digest(root / "src/evaluation/budget.py"),
        )
    window_frames = config.get("evidence_window_frames", 16)
    if window_frames not in (8, 16):
        raise ValueError("证据窗口只支持已定义的8或16帧")
    if window_frames == 8:
        from evaluation.short_video import ShortWindowMixin

        class ShortScorer(ShortWindowMixin, scorer_type):
            pass

        scorer_type = ShortScorer
        config = dict(
            config, short_scorer_sha256=file_digest(root / "src/evaluation/short_video.py")
        )
        if sequential_decode:
            raise ValueError("8帧使用独立单窗路径，不叠加16帧顺序解码规划器")
    if sequential_decode:
        from data.sequential_decode import SequentialDecodeMixin

        class SequentialScorer(SequentialDecodeMixin, scorer_type):
            pass

        scorer_type = SequentialScorer
        config = dict(
            config,
            evidence_decoder="sequential selected frames / original fallback",
            evidence_decoder_sha256=file_digest(root / "src/data/sequential_decode.py"),
        )
    done_path = output / f"completed_rank_{rank}.json"
    if done_path.exists():
        spec = json.loads((output / f"identity_rank_{rank}.json").read_text())
        done = json.loads(done_path.read_text())
        if (
            spec["config"] != config
            or spec["manifest_sha256"] != config_digest(frame.to_dict("records"))
            or spec["include_uniform"] != include_uniform
        ):
            raise ValueError("已完成分片配置/清单不匹配")
        if spec["models"] != {str(p): file_digest(p) for p in paths} or spec[
            "files"
        ] != video_file_states(root, frame):
            raise ValueError("已完成分片输入改变")
        for name, digest in spec["code"].items():
            if file_digest(root / name) != digest:
                raise ValueError("已完成分片实现改变")
        identity = config_digest(spec)
        if done["identity"] != identity or done["world_size"] != world_size:
            raise ValueError("完成分片身份不符")
        for i, row in enumerate(frame.itertuples()):
            if i % world_size == rank:
                checkpoint_read(output / "raw" / f"{i:06d}.json", identity, row.video_id)
        print(f"[复用完成分片] {output} rank={rank}", flush=True)
        return output
    if all(
        (output / "raw" / f"{i:06d}.json").exists() for i in range(rank, len(frame), world_size)
    ):
        # 重新分片后可能已有全部行而尚无该rank完成标记；原函数仍逐行验hash。
        score_evidence_manifest(
            root,
            config,
            frame,
            None,
        ...
        return output
    args = (
        (target["gs"], target["gt"], target["lt"], source["lt"].mean, config["runtime"]["device"])
        if baseline_only
        else (global_models, local, config["runtime"]["device"])
    )
    scorer = scorer_type(
        *args,
