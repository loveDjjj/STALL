...
def representation_vectors(global_windows, patch_windows):
    ...
    gd2 = g[:, 2:] - 2.0 * g[:, 1:-1] + g[:, :-2]
    values = dict(
        local_d1=torch.nn.functional.normalize(d1, dim=-1, eps=1e-12),
        local_d2=torch.nn.functional.normalize(d2, dim=-1, eps=1e-12),
        local_raw_d2=d2,
        local_d2_norm=torch.linalg.vector_norm(d2, dim=-1, keepdim=True),
...
def fit_representation_models(video_vectors):
    ...
    if set(video_vectors) != set(REPRESENTATIONS):
        raise ValueError("拟合表示集合不完整")
...
def fit_target_bank(root, dataset, fit_run, output):
    ...
    root = Path(root)
    fit_run = Path(fit_run)
    output = Path(output)
    output.mkdir(parents=True, exist_ok=False)
    fresh = pd.read_csv(fit_run / "prepared_fit.csv", keep_default_na=False)
    original = pd.read_csv(
        root / "data/manifests/active" / dataset / "fit.csv", keep_default_na=False
    ).set_index("video_id")
    values = {name: [] for name in REPRESENTATIONS}
    inputs = []
    for row in fresh.itertuples():
        cached = original.loc[row.video_id]
        old_path = root / cached.feature_asset
        new_path = Path(row.feature_asset)
        if (
            file_digest(old_path) != cached.feature_asset_sha256
            or file_digest(new_path) != row.feature_asset_sha256
        ):
        ...
        with (
            ...
            for key in ("raw_d2", "global_windows", "frame_indices"):
                if not np.array_equal(new[key], old[key]):
                    raise ValueError(f"{row.video_id}新旧{key}不同，须重提D1")
            ...
            values["local_d2_norm"].append(
                torch.linalg.vector_norm(raw, dim=-1, keepdim=True).numpy()
            )
            values["global_d2"].append(
                torch.nn.functional.normalize(gd2, dim=-1, eps=1e-12).reshape(-1, 1024).numpy()
            )
        inputs.append(
            dict(
                video_id=row.video_id,
                fresh_sha256=row.feature_asset_sha256,
                cached_sha256=cached.feature_asset_sha256,
            )
        )
...
def mean_whitening_controls(source, target, diagonal_target):
    ...
    if source.mean.shape != target.mean.shape:
        raise ValueError("源目标维度不同")
    return dict(
        source=source,
        target=target,
        target_mean_source_whitening=StableGaussianParams(
            target.mean, source.whitening, np.empty(0)
        ),
        source_mean_target_whitening=StableGaussianParams(
            source.mean, target.whitening, np.empty(0)
        ),
...
class RepresentationScorers:
    ...
    def __post_init__(self):
        ...
        for representation, models in self.models.items():
            if representation not in REPRESENTATIONS or not models:
                raise ValueError("未知或空表示模型")
            self.names[representation] = list(models)
            candidates = list(models.values())
            self.scorers[representation] = GaussianMeanCandidateScorerFloat64(
                candidates, candidates[0].mean, self.device
            )
