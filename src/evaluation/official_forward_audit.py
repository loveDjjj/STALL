...
def audit(root, output, device="cuda:0"):
    ...
    with np.load(parameter, allow_pickle=False) as z:
        params = {k: z[k].copy() for k in z.files}
    official = mod.STALL(device, params, load_dino=False)
    extractor = AlphaStallFeatureExtractor(
        device,
        dino_repo=str(root / cfg["encoder"]["repo"]),
        dino_weights=str(root / cfg["encoder"]["weights"]),
        pad_tail_batch=True,
    )
    jobs = []
    for domain in ("comgenvid", "genvideo", "videofeedback"):
        ...
        if domain != "comgenvid":
            frame = pd.read_csv(
                root / "data/manifests/short_video" / domain / "evaluation.csv",
                keep_default_na=False,
            )
            for _, part in frame.groupby(["subset", "source_model"]):
                r = part.sort_values("video_id").iloc[0]
                jobs.append((r, json.loads(r.downsample_idxs), 8))
    records = []
    for r, indices, length in jobs:
        ...
        official.model = extractor.model
        official.transform = extractor.transform
        ...
        if not np.array_equal(native32, g32):
            raise ValueError("默认batch32特征与官方前向不一致")
        a = official_numpy_score(g8[None], params)
        b = official._scores_from_embs(g8[None])
        c = official._scores_from_embs(g32[None])
        errors = [
            abs(a["global_spatial_raw"][0] - b["spat_ll_agg"][0]),
            abs(a["global_temporal_raw"][0] - b["temp_ll_agg"][0]),
            abs(a["final_score"][0] - b["final_score"][0]),
        ]
        if not all(e == 0 or np.isnan(e) for e in errors):
            raise ValueError("同特征官方评分不一致")
        records.append(
            dict(
                video_id=r.video_id,
                window_frames=length,
                feature_max_abs=float(np.max(abs(g8 - g32))),
                same_features_score_max_abs=float(np.nanmax(errors)),
                native32_feature_error=float(np.max(abs(native32 - g32))),
                batch8_score=float(b["final_score"][0]),
                batch32_score=float(c["final_score"][0]),
    ...
    paper_json(
        ...
        dict(
            status="completed",
            upstream_sha256=file_digest(source),
            parameters_sha256=file_digest(parameter),
            videos=len(records),
            scope="固定少量真实/生成视频同帧8/16帧；不是全数据batch敏感性保证",
...
if __name__ == "__main__":
    audit(Path.cwd(), Path("results/runs/complete23_official_forward_audit"))
