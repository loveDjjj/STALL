...
class ShortBudgetScorer(ShortWindowMixin, BudgetEvidenceScorer):
    pass
...
def measure(root, output, device="cuda:1"):
    ...
    for domain in ("genvideo", "videofeedback"):
        ...
        with np.load(target) as z:
            models = [
                StableGaussianParams(z[n + "_mean"], z[n + "_whitening"], np.empty(0))
                for n in ("gs", "gt", "lt")
            ]
        with np.load(root / "results/runs/paper_fit_source/gaussians.npz") as z:
            center = z["lt_mean"]
        scorer = ShortBudgetScorer(
            *models,
            center,
            device,
        ...
        frame = pd.read_csv(
            root / "data/manifests/short_video" / domain / "evaluation.csv", keep_default_na=False
        )
        sample = (
            frame.sort_values("video_id").groupby(["subset", "source_model"], sort=True).head(2)
        )
        ...
        gs = np.asarray([r["uniform"]["global_models"]["target"]["gs"] for r in cdf])
        gt = np.asarray([r["uniform"]["global_models"]["target"]["gt"] for r in cdf])
        lt = np.asarray(
            [r["selected"]["windows"][0]["representations"]["local_d2"]["target"] for r in cdf]
        )
        expected = pd.read_csv(
            root / "results/runs" / f"complete23_tables_{domain}_short8/video_scores.csv.gz",
        ...
        for selector, k in [
            ...
            first = sample.iloc[0]
            scorer.score_raw(
                root / first.video_path, json.loads(first.downsample_idxs), selector=selector, k=k
            )
            for repeat in range(2):
                for r in sample.itertuples():
                    ...
                    raw = scorer.score_raw(
                        root / r.video_path, json.loads(r.downsample_idxs), selector=selector, k=k
                    )
                    w = raw["windows"][0]
                    s = (
                        0.25 * float(percentile([w["global_models"]["target"]["gs"]], gs)[0])
                        + 0.25
                        * float(
                            percentile(
                                [w["global_models"]["target"]["gt"]],
                                gt,
                                allow_positive_infinity=True,
                            )[0]
                        )
                        + 0.5
                        * float(percentile([w["representations"]["local_d2"]["target"]], lt)[0])
                    )
                    torch.cuda.synchronize(device)
                    elapsed = time.perf_counter() - start
                    error = abs(s - float(expected.loc[r.video_id]))
                    if error > 1e-10:
                        raise ValueError("短视频主方法计时评分与完整证据不一致")
                    rows.append(
                        dict(
                            dataset=domain,
                            video_id=r.video_id,
                            ...
                            subset=r.subset,
                            selector=("fc" if selector == "feature_change" else "uniform") + str(k),
                            repeat=repeat,
                            total_seconds=elapsed,
                            observed_unique_frames=8,
                            coarse_frames=0,
...
if __name__ == "__main__":
    measure(Path.cwd(), Path("results/runs/complete23_short_runtime"))
