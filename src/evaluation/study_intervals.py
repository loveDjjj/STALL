...
def contrast(root, score_file, variant, baseline_file, baseline_variant, output, iterations=1000):
    ...
    output = Path(output)
    candidate = pd.read_csv(score_file, float_precision="round_trip")
    baseline = pd.read_csv(baseline_file, float_precision="round_trip")
    ...
    metadata = dict(
        candidate_sha256=file_digest(score_file),
        baseline_sha256=file_digest(baseline_file),
        candidate=variant,
    ...
    if output.exists():
        ...
        if manifest["inputs"] != metadata or manifest["status"] != "completed":
            raise ValueError("已有区间输出不匹配")
        for name, digest in manifest["files"].items():
            if file_digest(output / name) != digest:
                raise ValueError("已有区间产物hash改变")
        return
