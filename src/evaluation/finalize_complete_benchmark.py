...
def finalize(root,directory):
    ...
    if verification['status']!='verified' or verification['cells']!=23:raise ValueError('完整实验尚未验收')
    ...
    for d in ('comgenvid','videofeedback','genvideo'):
        ...
        timings.append(pd.read_csv(p).assign(window_frames=16));sources[str(p)]=file_digest(p)
    p=root/'results/runs/complete23_short_runtime/timings.csv'
    timings.append(pd.read_csv(p).assign(window_frames=8));sources[str(p)]=file_digest(p)
    timing=pd.concat(timings,ignore_index=True)
    timing.to_csv(directory/'runtime_videos.csv',index=False)
    timing.groupby(['dataset','window_frames','selector']).agg(n_measurements=('total_seconds','size'),
        n_clip_ids=('video_id','nunique'),mean_seconds=('total_seconds','mean'),median_seconds=('total_seconds','median'),
        mean_unique_frames=('observed_unique_frames','mean'),peak_gpu_mib=('peak_gpu_allocated_mib','max')).reset_index().to_csv(directory/'runtime_summary.csv',index=False)
    lines=['# 完整23单元论文实验结果','',f"协议：{original['scope']}。所有主比较及消融覆盖VideoFeedback 11、GenVideo 10、ComGenVid 2单元。",
    ...
    for study in STUDIES:
        ...
        lines += ['## '+study,'','| 方法 | VideoFeedback AUC/AP | GenVideo AUC/AP | ComGenVid AUC/AP | Average AUC/AP | Macro-3 AUC/AP |','| --- | ---: | ---: | ---: | ---: | ---: |']
        for (variant,mode),part in table.groupby(['variant','observation'],sort=False):
            ...
            for domain in ('videofeedback','genvideo','comgenvid','Average','Macro-3'):
                r=part[part.dataset==domain].iloc[0];values.append(f'{r.auc:.3f}/{r.real_positive_ap:.3f}')
    ...
    original.update(status='completed',verification=verification,tests_passed=52,runtime_sources=sources,
        finalized_code_sha256=file_digest(Path(__file__)),files={p.name:file_digest(p) for p in directory.iterdir() if p.is_file() and p.name!='manifest.json'})
    paper_json(directory/'manifest.json',original)


if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('directory');a=p.parse_args();finalize(Path.cwd(),a.directory)
