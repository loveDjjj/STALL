...
class SharedEvidenceScorer(RawVideoScorer):
    ...
    def __init__(self, global_models, representation_models, device, **kwargs):
        ...
        super().__init__(
            target[0], target[1], representation_models["local_d2"]["target"], device, **kwargs
        )
    ...
    def score_dense(self, plan, frames):
        ...
        glob = {
            name: score_global_raw(g, gs, gt, device=self.device)
            for name, (gs, gt) in self.global_models.items()
        }
        if self.device.startswith("cuda"):
            torch.cuda.synchronize(self.device)
        ...
        for i, w in enumerate(windows):
            records.append(
                dict(
                    ...
                    change=w.change,
                    global_models={
                        name: dict(gs=float(x.spatial[i]), gt=float(x.temporal_t1[i]))
                        for name, x in glob.items()
                    },
                    representations={
                        name: {model: float(x[i]) for model, x in scores.items()}
...
def score_evidence_manifest(
    root, config, frame, scorer, output, model_paths, *, include_uniform=False, rank=0, world_size=1
):
    ...
    if marker.exists():
        if json.loads(marker.read_text()) != payload:
            raise ValueError("证据任务身份已改变")
    else:
        paper_json(marker, payload)
    pending = []
    ...
    start = time.perf_counter()
    for completed, ((index, row), raw) in enumerate(
        prefetched_raw_scores(root, config, scorer, pending, include_uniform=include_uniform), 1
    ):
        checkpoint_write(
            output / "raw" / f"{index:06d}.json",
            identity,
            dict(video_id=row.video_id, evidence=raw),
        )
    ...
    paper_json(
        ...
        dict(
            ...
            total_rows=sum(i % world_size == rank for i in range(len(frame))),
            elapsed_seconds=time.perf_counter() - start,
        ),
    )
