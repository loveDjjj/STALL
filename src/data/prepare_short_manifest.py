...
def one_second(row):
    ...
    row["1_sec_idxs"] = row["downsample_idxs"]
    row["2_sec_idxs"] = ""
    row["original_video_id"] = row["video_id"]
    row["video_id"] += ":duration1"
...
def prepare(root, domain):
    ...
    fields = active.columns.tolist()
    candidates = []
    if domain == "genvideo":
        old = pd.read_csv(
            root / "cache/contracts/data/manifests/development/genvideo_evaluation.csv",
        ...
        for key, m in sorted(metadata.items()):
            ...
            a, b = map(float, probe["avg_frame_rate"].split("/"))
            fps = a / b
            ...
            while round(fps * j / 8) < count:
                indices.append(round(fps * j / 8))
                j += 1
            if len(indices) < 8:
                continue
            relative = str(path.relative_to(root))
            vid = f"{domain}:{relative}"
            row = {k: "" for k in fields}
            row.update(
                video_path=relative,
                subset="annotated",
                source_model="Hotshot-XL",
                fps=fps,
                num_frames=count,
                ...
                source_group=vid,
                generator="Hotshot-XL",
                dynamic_degree=m["dynamic degree"],
            )
            candidates.append(one_second(row))
    fake = pd.DataFrame(candidates)
    real = pd.DataFrame(
        [one_second(row) for row in active[active.subset == "real"].to_dict("records")]
    )
    pairs = []
    for generator, g in fake.groupby("source_model"):
        count = min(len(g), len(real))
        sources = real.source_model.nunique()
        count = count // sources * sources
        picked_fake = g.sample(n=count, random_state=42)
        picked_real = pd.concat(
            [r.sample(n=count // sources, random_state=42) for _, r in real.groupby("source_model")]
        )
        for row in pd.concat([picked_real, picked_fake]).to_dict("records"):
            pairs.append(
                {k: row[k] for k in ("video_id", "dataset", "subset")} | {"generator": generator}
            )
    # 只评分正式配对涉及的唯一身份；fake余量清单另外保存，不按得分挑样本。
    pairs = pd.DataFrame(pairs)
    frame = pd.concat([real, fake], ignore_index=True)
    frame = frame[frame.video_id.isin(set(pairs.video_id))].fillna("")
    out = root / "data/manifests/short_video" / domain
    ...
    paper_json(
        ...
        dict(
            status="prepared",
            rule="8fps 1sec official seed42 first draw; real/fake same frames and pool; balanced seed42",
            unique_videos=len(frame),
            pair_rows=len(pairs),
            generators=sorted(fake.source_model.unique()),
            source_evaluation_sha256=file_digest(
                root / "data/manifests/active" / domain / "evaluation.csv"
            ),
...
if __name__ == "__main__":
    ...
    p.add_argument("domain", choices=["genvideo", "videofeedback"])
