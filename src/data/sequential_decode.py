...
def decode_sequential(path, frame_indices):
    ...
    indices = validate_indices(sorted(set(requested)))
    if not indices:
    ...
    try:
        ...
        capture.release()
    if wanted - set(found):
        return decode_bounded(path, requested)
...
class SequentialDecodeMixin:
    ...
    def prepare_coarse(
        ...
        *,
        ...
        if len(indices) < 16 or k not in (1, 2, 3) or selector not in ("uniform", "feature_change"):
            raise ValueError("观察参数错误")
        ...
        if requested:
            ...
            uniform_pair = (
                (uniform, prepared[[positions[i] for i in uniform.frame_indices]])
                if uniform
                else None
            )
        ...
        return dict(
            ...
            coarse=[coarse_frames[i : i + 8] for i in range(0, len(coarse), 8)],
