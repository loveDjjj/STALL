...
def load_active_groups(root, manifest_root, scores):
    ...
    root=Path(root);manifest_root=Path(manifest_root)
    registry=json.loads((manifest_root/'manifest.json').read_text())
    if registry['status']!='completed':raise ValueError('活跃数据清单未完成')
    frames=[];provenance={}
    for domain in sorted(scores.dataset.unique()):
        ...
        if required-set(frame) or frame.video_id.duplicated().any():raise ValueError('源组清单字段/身份错误')
    ...
    if not set(scores.video_id).issubset(all_groups.index):raise ValueError('评分视频缺失源组身份')
    aligned=all_groups.loc[scores.video_id]
    for column in ('dataset','subset','source_model'):
        if not (aligned[column].to_numpy()==scores[column].to_numpy()).all():raise ValueError('评分与源组清单标签/来源不一致')
    if aligned.source_group.isna().any() or aligned.source_group.astype(str).str.len().eq(0).any():raise ValueError('源组为空')
