"""为原始视频目录构建 Alpha STALL manifest（例如 GenVideo）。

既可作为独立脚本使用，也可作为模块导入。

用法：
    conda run -n stall python scripts/build_manifest.py --real-dir /data/GenVideo/real --fake-dir /data/GenVideo/fake --output data/manifests/custom.csv
    conda run -n stall python scripts/build_manifest.py --real-dir /data/GenVideo/real --fake-dir /data/GenVideo/fake --output data/manifests/custom.csv --debug 5
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "为原始视频目录构建 Alpha STALL manifest。"
