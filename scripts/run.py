...
def references(config, domains):
    ...
    if manifest["status"] != "completed":
        raise ValueError("参考包尚未完成")
    registry = {r["dataset"]: r for r in manifest["references"]}
    result = {}
    hashes = {}
    for domain in domains:
        if domain not in registry:
            raise ValueError(f"没有显式目标参考：{domain}")
        path = resolve(config["reference"]["directory"]) / f"{domain}.npz"
        digest = file_digest(path)
        if digest != registry[domain]["sha256"]:
            raise ValueError("参考包哈希不匹配")
        bundle = load_bundle(path)
        if bundle.metadata["dataset"] != domain:
            raise ValueError("参考包目标域不匹配")
        if config["selection"]["name"] != bundle.metadata.get(
            "selector", "feature_change"
        ) or config["selection"]["k"] != bundle.metadata.get("requested_k", 3):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="严格点分覆盖，例如runtime.device=cuda:1",
    )
    ...
    for role in ("fit", "cdf", "evaluation", "threshold"):
        fit.add_argument(
            f"--{role}-manifest",
            type=Path,
            help="默认使用活跃清单；评测/阈值仅检查隔离",
        )
    fit.add_argument(
        "--stop-after",
        choices=("gaussian", "cdf"),
        default="cdf",
        help="gaussian可暂停，恢复到cdf后才有可部署参考包",
    )
    ...
    evaluate.add_argument(
        "--resume", action="store_true", help="同评分/配对/源码恢复未完成评价发布"
    )
    bootstrap = commands.add_parser(
        "bootstrap", help="已有候选/基线分数的源组配对区间，不重新计算模型"
    )
...
def main(argv=None):
    """解析参数并分派原有执行流程；argv 为 None 时读取进程命令行。"""
    import pandas as pd
    ...
    if getattr(args, "resume", False) and args.action != "evaluate" and not args.output:
        parser.error("--resume必须显式指定原--output目录")
    config = load_paper_config(args.config, args.set)
    if args.action == "study":
        required = {
            "representation-fit": ["fit_run"],
            "evidence": ["manifest"],
            "tables": ["evidence_run", "cdf_run"],
            "official-tables": ["evidence_run"],
        }.get(args.stage, [])
        for key in required:
            if getattr(args, key) is None:
                parser.error(f"{args.stage}需要--{key.replace('_', '-')}")
        if not 0 <= args.rank < args.world_size:
            parser.error("rank须在[0,world-size)内")
        if args.limit is not None and (args.limit < 1 or args.stage != "evidence"):
            parser.error("limit仅用于evidence正整数小批检查")
        if args.dry_run:
            budget = (
                args.baseline_only
                if args.baseline_only is not None
                else (config["selection"]["name"], config["selection"]["k"])
                in {("feature_change", 1), ("uniform", 3)}
            )
            print(
                json.dumps(
                    dict(
                        ...
                        config=config,
                        baseline_only=budget,
                        note="规划检查；没有加载GPU或生成完成结果",
                    ),
                    ensure_ascii=False,
                    indent=2,
                )
        ...
        if args.stage == "source-fit":
            fit_source_bank(ROOT, config, resolve(args.output))
        elif args.stage == "representation-fit":
            fit_target_bank(ROOT, args.dataset, resolve(args.fit_run), resolve(args.output))
            ...
            run_shared_evidence(
                ...
                limit=args.limit,
                sequential_decode=args.sequential_decode,
            ...
            aggregate_domain(
                ROOT,
                args.dataset,
                resolve(args.evidence_run),
                resolve(args.cdf_run),
                resolve(args.output),
            )
        elif args.stage == "official-tables":
            aggregate_official(
                ROOT,
                args.dataset,
                resolve(args.evidence_run),
    ...
    if args.action == "components":
        ...
        if source_config["method"] != canonical or config["method"] != canonical:
            raise ValueError("组件分析须以等权完整主线为源，不接受更换权重")
        ...
        if source_manifest["inputs"].get("references") != hashes:
            raise ValueError("组件源run与当前目标参考身份不符")
        pairs = pd.read_csv(args.pairs, dtype={"video_id": str})
        tables = component_ablation_tables(scores, pairs)
        inputs = dict(
            source=source,
            pairs=dict(path=str(args.pairs.resolve()), sha256=file_digest(args.pairs)),
            references=hashes,
        ...
        with PaperLog(directory):
            try:
                for name, table in tables.items():
                    table.to_csv(
                        directory / (name + ".csv.gz" if name == "video_scores" else name + ".csv"),
                        index=False,
                    )
                ...
                paper_json(
                    ...
                    dict(
                        full="0.5 Global + 0.5 Local",
                        global_only="Global",
                        local_only="Local",
                        without_gs="0.5 GT + 0.5 Local",
                        without_gt="0.5 GS + 0.5 Local",
                ...
                paper_json(
                    ...
                    dict(status="failed", stage="components", error=str(exc)),
                )
                raise
        return
    if args.action == "export":
        ...
        with PaperLog(directory):
            try:
                ...
                print(
                    f"[完成:export] {directory}/references；显式配置reference.directory/manifest使用",
                    flush=True,
                )
            except BaseException as exc:
                paper_json(
                    directory / "status.json",
                    dict(status="failed", stage="export", error=str(exc)),
                )
                raise
        return
    if args.action == "bootstrap":
        ...
        baseline, baseline_info = read_paper_scores(
            args.baseline_run.resolve(), args.baseline_variant
        )
        pairs = pd.read_csv(args.pairs)
        groups, group_inputs = load_active_groups(
            ROOT, resolve(config["data"]["manifests"]), candidate
        )
        inputs = dict(
            candidate=candidate_info,
            baseline=baseline_info,
            pairs=dict(path=str(args.pairs.resolve()), sha256=file_digest(args.pairs)),
            source_manifests=group_inputs,
            seed_policy="SeedSequence(config seed, stable dataset hash)",
            scope="compare frozen scores; no rescoring",
        )
        if args.dry_run:
            print(json.dumps(inputs, indent=2))
            return
        ...
        with PaperLog(directory):
            try:
                ...
                delta, interval = paired_source_contrast(
                    candidate,
                    baseline,
                    pairs,
                    groups,
                ...
                paper_json(
                    directory / "status.json",
                    dict(status="failed", stage="bootstrap", error=str(exc)),
                )
                raise
        return
    ...
    if args.action == "fit":
        ...
        paths = {
            role: (
                getattr(args, f"{role}_manifest")
                or base
                / ("vatex" if role in ("cdf", "threshold") else args.dataset)
                / f"{role}.csv"
            ).resolve()
            for role in ("fit", "cdf", "evaluation", "threshold")
        }
        fit_frame, frame, inputs = prepare_fit_inputs(
            ROOT,
            config,
            args.dataset,
            *(paths[r] for r in ("fit", "cdf", "evaluation", "threshold")),
        )
        ...
        path = (
            ...
            if args.manifest
            else resolve(config["data"]["manifests"]) / args.dataset / "evaluation.csv"
        )
        ...
        if required - set(frame):
            raise ValueError("评分manifest字段不完整")
        if frame.video_id.duplicated().any() or not frame.dataset.eq(args.dataset).all():
            raise ValueError("manifest身份重复或目标域不符")
        if not set(frame.subset).issubset({"real", "annotated"}):
            raise ValueError("评分manifest标签无效；无标签单视频使用predict")
        if args.limit is not None:
            if args.limit < 1:
                raise ValueError("limit必须为正")
            frame = frame.head(args.limit)
        if frame.empty:
            raise ValueError("评分manifest为空")
        for value in frame.downsample_idxs:
            if len(validate_indices(json.loads(value))) < 16:
                raise ValueError("manifest含不足16帧的视频")
        domains = [args.dataset]
        inputs["manifest"] = dict(
            path=str(path),
            sha256=file_digest(path),
            selected_videos=len(frame),
            limit=args.limit,
        )
        ...
        if args.indices_json:
            ...
            inputs["indices"] = dict(
                path=str(args.indices_json.resolve()),
                sha256=file_digest(args.indices_json),
            )
        else:
            ...
            inputs["probe"] = probe
        if len(indices) < 16:
            raise ValueError("短视频不足16互异帧，不填帧")
        domains = [args.dataset]
        inputs["video"] = dict(path=str(path), sha256=file_digest(path))
    if args.action != "fit":
        bundles, hashes = references(config, domains)
        inputs["references"] = hashes
    if args.dry_run:
        if getattr(args, "resume", False):
            resume_paper_run(ROOT, config, inputs, args.output, args.action)
        brief = {
            key: dict(count=len(value), sha256=config_digest(value))
            if key in ("fit_assets", "video_files", "fit_video_files", "cdf_video_files")
            else value
            for key, value in inputs.items()
        }
        print(
            json.dumps(
                dict(action=args.action, config=config, inputs=brief),
                indent=2,
                ensure_ascii=False,
            )
        )
        return
    if (
        args.action in ("score", "predict")
        and file_digest(resolve(config["encoder"]["weights"]))
        != config["encoder"]["weights_sha256"]
    ):
        raise ValueError("DINO权重不匹配")
    directory = (
        args.output.resolve()
        if getattr(args, "resume", False)
        else create_paper_run(ROOT, config, sys.argv, inputs, args.output)
    )
    ...
    try:
        ...
        if args.action == "fit":
            ...
            def progress(stage, index, total, vid):
                paper_json(
                    ...
                    dict(stage=stage, completed=index, total=total, video_id=vid),
                )
                if index % 16 == 0 or index == total:
                    print(f"[fit:{stage}] {index}/{total}", flush=True)

            result = run_reference_fit(
                ROOT,
                config,
                args.dataset,
                fit_frame,
            ...
            def progress(index, vid):
                if index % 100 == 0 or index == total:
                    paper_json(
                        directory / "progress.json",
                        dict(completed=index, total=total, video_id=vid),
                    )
                    print(f"[replay] {index}/{total}", flush=True)

            scores = replay_checkpointed(
                frame, bundles, hashes, config["method"], directory, identity, progress
            )
            ...
            if args.action == "predict":
                scorer = VideoScorer(
                    bundles[args.dataset],
                    config["runtime"]["device"],
                    dino_repo=str(resolve(config["encoder"]["repo"])),
                    dino_weights=str(resolve(config["encoder"]["weights"])),
                )
                ...
                paper_json(
                    ...
                    dict(
                        video_path=str(path),
                        dataset=args.dataset,
                        reference_sha256=hashes[args.dataset],
                        threshold=None,
                ...
                def progress(index, vid):
                    paper_json(
                        directory / "progress.json",
                        dict(completed=index, total=len(frame), video_id=vid),
                    )
                    if index % 16 == 0 or index == len(frame):
                        print(f"[score] {index}/{len(frame)}", flush=True)

                rows, windows = score_manifest_distributed(
                    ROOT,
                    config,
                    frame,
                    bundles[args.dataset],
                ...
                if video_file_states(ROOT, frame) != inputs["video_files"]:
                    raise ValueError("评分期间输入视频集合状态改变")
        ...
        paper_json(
            directory / "status.json",
            dict(status="failed", stage=args.action, error=str(exc)),
        )
        raise
    finally:
