...
for dataset in comgenvid videofeedback genvideo; do
  ...
  CUDA_VISIBLE_DEVICES="$gpu_index" conda run --no-capture-output -n stall \
    python tools/score_u0_locked_calibration_reference.py \
    --dataset "$dataset" \
  ...
  conda run --no-capture-output -n stall \
    python tools/finalize_u0_locked_calibration_reference.py \
    --dataset "$dataset" \
    --output-dir "$output_dir" \
    --num-shards 2 \
    --shard-index "$shard_index"
  if [[ $score_status -ne 0 ]]; then
    echo "calibration scorer exited $score_status after complete checkpoints; finalizer verified recovery" >&2
  fi
done
