...
给定视频 $V=\{I_t\}_{t=1}^{T}$，目标是输出真实性分数 $S(V)\in[0,1]$，数值越大表示视频越符合真实视频统计规律，数值越小表示越可能为生成视频。检测器只允许使用预先指定的真实视频建立参考统计，不使用生成视频训练分类器，也不在推理时读取生成器来源、测试标签或测试批次真假组成。若投稿协议要求严格外部校准，真实校准集应与目标测试集完全隔离；本 release 的具体校准来源在实验设置和讨论中单独标注。
...
\end{equation}
在本 release 中 $D=1024$，$N=14\times14$。全局序列 $G=\{g_t\}_{t=1}^{T}$ 用于复现 \stall 的全局空间和全局时序分支；局部序列 $P=\{P_t\}_{t=1}^{T}$ 用于建立 patch 空间和 patch 二阶时序分支。由于两类证据来自同一次前向传播，局部扩展不需要额外训练新的 backbone。
...
\end{equation}
Local whitening、窗口级 CDF 和视频级 CDF 均仅由目标域独立真实视频估计；生成视频不参与这些统计量的拟合或校准，锁定运行也不再据此调整权重。固定的 $\alpha$、$\beta$、$K$ 及总体结构是在三个开发基准上查看生成视频结果后冻结的，因此这些基准不属于未触碰的确认集。正式实现将 whitening 和高斯似然阶段转换为 float64，经验 CDF 使用稳定排序和右闭合 tie 规则。该实现不改变 DINOv3 的 float32 特征，只消除高维 whitening 对外层 batch shape 的数值依赖。
